import { useEffect, useState } from "react";
import { useTheme } from "@mui/material/styles";
import Box from "@mui/material/Box";
import OutlinedInput from "@mui/material/OutlinedInput";
import InputLabel from "@mui/material/InputLabel";
import MenuItem from "@mui/material/MenuItem";
import FormControl from "@mui/material/FormControl";
import Select from "@mui/material/Select";
import Chip from "@mui/material/Chip";

const ITEM_HEIGHT = 48;
const ITEM_PADDING_TOP = 8;
const MenuProps = {
  PaperProps: {
    style: {
      maxHeight: ITEM_HEIGHT * 4.5 + ITEM_PADDING_TOP,
      width: 250,
    },
  },
};

const names = [
  "Aria condizionata",
  "Impianto di allarme",
  "Videocitofono",
  "Ascensore interno",
  "Pavimenti in parquet",
  "Tende elettriche",
  "Illuminazione a LED",
  "Sistema di irrigazione automatico per il giardino",
  "Sistema di filtraggio dell'acqua",
  "Camino o stufa a legna",
  "Sistema di riciclo dell'aria",
  "Piscina o vasca idromassaggio",
  "Sistema di domotica",
  "Pannelli solari per l'acqua calda",
  "Serrande elettriche",
  "Citofono videocamera",
  "Sistema di isolamento acustico",
  "Pareti insonorizzate",
];
function getStyles(name, rooms, theme) {
  return {
    fontWeight: rooms.indexOf(name) === -1 ? theme.typography.fontWeightRegular : theme.typography.fontWeightMedium,
  };
}
const ChipOtherCharacteristics = (props) => {
  const theme = useTheme();
  const [otherCharacteristics, setOtherCharacteristics] = useState([]);

  const handleChange = (event) => {
    const {
      target: { value },
    } = event;
    setOtherCharacteristics(typeof value === "string" ? value.split(",") : value);
  };
  useEffect(() => {
    props.setBody({ ...props.body, otherCharacteristics: otherCharacteristics });
  }, [otherCharacteristics]);

  return (
    <div>
      <FormControl sx={{ m: 1, width: 300 }}>
        <InputLabel id="demo-multiple-chip-label">Caratteristiche</InputLabel>
        <Select
          labelId="demo-multiple-chip-label"
          id="demo-multiple-chip"
          multiple
          value={otherCharacteristics}
          onChange={handleChange}
          input={<OutlinedInput id="select-multiple-chip" label="Chip" />}
          renderValue={(selected) => (
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
              {selected.map((value) => (
                <Chip key={value} label={value} />
              ))}
            </Box>
          )}
          MenuProps={MenuProps}
        >
          {names.map((name) => (
            <MenuItem key={name} value={name} style={getStyles(name, otherCharacteristics, theme)}>
              {name}
            </MenuItem>
          ))}
        </Select>
      </FormControl>
    </div>
  );
};
export default ChipOtherCharacteristics;
